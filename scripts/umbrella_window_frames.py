...
ap.add_argument("--frame-stride",dest="frame_stride",type=int,default=0)
...
colvar=[]; frames=[]; nprod=int(a.ns_prod*250000); rec=250   # record every 1 ps
# Frame capture for the ensemble-fluctuation test: the loop already materializes
# full coordinates every 1 ps to evaluate r_DA, so storing a uniform stride of
# them costs nothing extra in simulation time. Stride is fixed by --frame-stride.
for done in range(0,nprod,rec):
    ...
    if a.frame_stride>0 and (len(colvar)-1) % a.frame_stride == 0:
        frames.append(p.astype('float32'))
colvar=np.array(colvar)
if frames:
    np.save(a.out+"_frames.npy", np.asarray(frames))
    print(f"saved {len(frames)} frames -> {a.out}_frames.npy")
