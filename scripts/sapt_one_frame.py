...
TAG, CLAMP, FRAME = sys.argv[1], sys.argv[2], int(sys.argv[3])
DIR = Path('/home/liang/Workspace/WritePaper/CatalysisQuamBio/results/ensemble_fluctuation')
with open(DIR/f'{TAG}_{CLAMP}_frames_geometry.json') as f:
    geom = json.load(f)['frames'][FRAME]
...
for delta in DELTAS:
    ...
    outfile = DIR/f'sapt_{TAG}_{CLAMP}_f{FRAME:03d}_{delta:+.2f}.out'
...
if len(results) >= 3:
    ...
    out = dict(TAG=TAG, clamp=CLAMP, frame=FRAME, wall_residue='see frames_geometry',
    ...
    with open(DIR/f'{TAG}_{CLAMP}_f{FRAME:03d}.json','w') as f:
        json.dump(out, f, indent=2)
    print(f'wrote {DIR/f"{TAG}_{CLAMP}_f{FRAME:03d}.json"}')
