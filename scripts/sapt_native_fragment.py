...
for delta in DELTAS:
    ...
    outfile = DIR/f'sapt_{TAG}_native_{delta:+.2f}.out'
...
if len(results) >= 3:
    ...
    out = dict(TAG=TAG, wall_residue=geom['wall_residue'],
    ...
    with open(DIR/f'{TAG}_native_result.json','w') as f:
        json.dump(out, f, indent=2)
    print(f'wrote {DIR/f"{TAG}_native_result.json"}')
