...
WIN=/home/liang/Workspace/WritePaper/CatalysisQuamBio/scripts/umbrella_window_frames.py
cd /home/liang/Workspace/WritePaper/CatalysisQuamBio/md/mcpb

OUT=/home/liang/Workspace/WritePaper/CatalysisQuamBio/results/ensemble_fluctuation
mkdir -p "$OUT"
LOG="$OUT/campaign.log"
echo "=== ensemble-fluctuation campaign start $(date -Iseconds) ===" | tee -a "$LOG"
...
run_clamp () {
    ...
    lbl="${tag}_r${r0/./}"
    if [[ -f "$OUT/${lbl}_colvar.dat" && -f "$OUT/${lbl}_frames.npy" ]]; then
        echo "  skip $lbl (exists)" | tee -a "$LOG"; return
    fi
    echo "--- $(date -Iseconds) $tag clamp r_DA=$r0 A ---" | tee -a "$LOG"
    for att in 1 2 3; do
        [[ -f "$OUT/${lbl}_colvar.dat" && -f "$OUT/${lbl}_frames.npy" ]] && break
        timeout 5400 "$SL" "$WIN" --prmtop "$prm" --seed "$rst" \
            --donor "$d" --acceptor "$a" --r0 "$r0" --k "$KSTIFF" \
            --ns-eq 0.3 --ns-prod 3.0 --frame-stride 75 --out "$OUT/$lbl" \
            --rngseed $((RANDOM+1)) 2>&1 | tee -a "$LOG" | tail -3
        [[ -f "$OUT/${lbl}_colvar.dat" && -f "$OUT/${lbl}_frames.npy" ]] && { echo "  ok $lbl" | tee -a "$LOG"; break; }
        echo "  attempt $att failed; retry" | tee -a "$LOG"; sleep 20
    done
    [[ -f "$OUT/${lbl}_colvar.dat" && -f "$OUT/${lbl}_frames.npy" ]] || echo "  UNRECOVERED $lbl" | tee -a "$LOG"
}
...
echo "" | tee -a "$LOG"
echo "=== ensemble-fluctuation campaign done $(date -Iseconds) ===" | tee -a "$LOG"
for r0 in 2.55 3.40; do
